...
Orchestrates TelemetrySnapshot processing, target evaluations, local Qwen LLM reasoning,
dynamic candidate generation with comfort guardrails, discrete action translation,
rate-limiting, anti-oscillation, safety validation, self-correction, and tool request generation.
"""
...
class AgentOrchestrator:
    """
    Cognitive Agent Orchestrator connecting telemetry state to local open-source LLM reasoning,
    dynamic candidate action generation, guardrail enforcement, and custom agent tool calls.
    """
    ...
    # Absolute Thermostat Safety Boundaries
    ...
    def __init__(self, llm_client: Optional[LocalQwenLLMClient] = None):
        ...
        self.last_adjustment_direction: Dict[str, str] = {}  # {zone: "UP" / "DOWN"}
        self.intervals_since_adjustment: Dict[str, int] = {}  # {zone: interval_count}
    ...
    def generate_allowed_candidates(self, eval_state: Dict[str, Any]) -> List[str]:
        """
        Dynamically generates safe, thermodynamically valid candidate actions for the current state.
        Enforces comfort guardrails (-0.5 <= PMV <= +0.5) and comfort buffer (-0.3 <= PMV <= +0.3).
        """
        pmv = eval_state["pmv"]
        ...
        if is_occupied:
            # Occupied Comfort Guardrails
            if pmv < -0.5:
                # TOO_COLD: Block RELAX_HEATING. Allow RESTORE_HEATING if below ceiling.
                if htg_sp + 0.5 <= self.MAX_HEATING_CEILING:
                    candidates.append("RESTORE_HEATING_0_5C")
            elif pmv > +0.5:
                # TOO_HOT: Block RELAX_COOLING. Allow RESTORE_COOLING if above floor.
                if clg_sp - 0.5 >= self.MIN_COOLING_FLOOR:
                    candidates.append("RESTORE_COOLING_0_5C")
            elif -0.3 <= pmv <= +0.3:
                # Comfortably inside buffer: Energy Optimization allowed if carbon HIGH or demand peak
                if hvac_mode == "HEATING" and (carbon_status == "HIGH" or peak_status in ["NEAR_LIMIT", "ABOVE_LIMIT"]):
                    if htg_sp - 0.5 >= self.MIN_HEATING_FLOOR:
                        candidates.append("RELAX_HEATING_0_5C")
                elif hvac_mode == "COOLING" and (carbon_status == "HIGH" or peak_status in ["NEAR_LIMIT", "ABOVE_LIMIT"]):
                    if clg_sp + 0.5 <= self.MAX_COOLING_CEILING:
                        candidates.append("RELAX_COOLING_0_5C")
            else:
                # Comfort Buffer Edge (-0.5 to -0.3 or +0.3 to +0.5): Prefer MAINTAIN
                if htg_sp < 21.0 and pmv < -0.3:
                    candidates.append("RESTORE_HEATING_0_5C")
                elif clg_sp > 24.0 and pmv > +0.3:
                    candidates.append("RESTORE_COOLING_0_5C")
        else:
            # Unoccupied Setback Policy: Allow conservative energy relaxation
    ...
    def translate_candidate_action(
        ...
        Returns (action_type, target_htg_sp, target_clg_sp).
        """
    ...
    def process_snapshot(
        ...
        """
        Executes full cognitive workflow:
        Telemetry -> Evaluated State -> Candidate Generation -> Local Qwen -> Discrete Action Translation -> Safety Validation -> Tool Request.
        """
        ...
        while attempts < max_attempts:
            ...
            # Enforce that selected candidate must be in allowed candidates list
            if raw_candidate_action not in allowed_candidates:
                raw_candidate_action = "MAINTAIN"

            # Translate discrete candidate into numeric setpoints
            ...
            # Safety Validation if action is ADJUST_SETPOINTS
        ...
        # Fallback to MAINTAIN if attempts exhausted without valid decision
        ...
        # Log decision to JSONL with run_id
        log_record = {
            "run_id": run_id,
            "timestamp": time.time(),
            "simulation_time": eval_state["simulation_time"],
